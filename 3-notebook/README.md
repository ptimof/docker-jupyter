...
```
docker run -d -p 443:8888 -e "PASSWORD=MakeAPassword" ptimof/notebook
```
...
```
docker run -v /path/to/hostname.pem:/key.pem -d -p 443:8888 -e "PASSWORD=pass" ptimof/notebook
```
...
Example:
```
docker run -d -p 80:8888 -e "PASSWORD=MakeAPassword" -e "USE_HTTP=1" ptimof/notebook
```

## Persisting notebooks

You can persist your notebooks by mounting your home directory to the docker container:
```
docker run -d -p 443:8888 -e "PASSWORD=MakeAPassword" -v /home/ptimof:/notebooks ptimof/notebook
```

## Browser notes

As of this writing, Safari is not supported with self-signed certificates.
